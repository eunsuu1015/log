...
| 시간대별 히트맵 그리드 | 0~23시 24칸, 6열×4행. 비율 기반 4단계 색상(≤25%→연→진)을 테마 강조 색상(`cs.primary`) 기준으로 생성 — 더보기 > 테마에서 색상을 바꾸면 즉시 반영. 최다 방문 셀에 primary 테두리 강조. 상단 피크 시간 요약 카드. 헤더에 범례·"자세히 보기 ↑" 버튼 포함 |
...
| 설정 | 테마 | 앱 강조 색상(메인 컬러) 선택 — 빨강·주황·노랑·초록(기본값)·하늘·파랑·남색·보라·회색·검정 10종. `themeAccentProvider` → `shared_preferences` 저장. `AppTheme.light()/dark()`가 선택한 색상으로 primary 계열을 교체해 앱 전체(버튼·FAB·강조 텍스트 등)에 즉시 반영. 초록 선택 시 기존 디자인과 완전히 동일 |
