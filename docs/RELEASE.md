...
```json
{
  ...
  "ADMOB_INTERSTITIAL_ANDROID": "ca-app-pub-xxxx/xxxx",
  "ADMOB_INTERSTITIAL_IOS":     "ca-app-pub-xxxx/xxxx",
  "ADMOB_NATIVE_ANDROID":       "ca-app-pub-xxxx/xxxx",
  "ADMOB_NATIVE_IOS":           "ca-app-pub-xxxx/xxxx",
  "FEEDBACK_URL":               "https://...",
...
> `--dart-define-from-file=secrets.json` 없이 빌드하면 AdMob ID·피드백 URL·개인정보처리방침 URL이 모두 테스트값 또는 빈 문자열로 배포된다.
...
- [ ] `secrets.json` 실제 AdMob ID 입력 확인
- [ ] `local.properties`에 `admob.app.id` 설정 확인
- [ ] `--dart-define-from-file=secrets.json` 포함 빌드 확인
- [ ] `QA_CHECKLIST.md` 🔴 크리티컬 항목 전체 통과 확인
- [ ] 개인정보처리방침 URL 정상 연결 확인
- [ ] 피드백 보내기 URL 정상 연결 확인
